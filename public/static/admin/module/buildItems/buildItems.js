...
layui.define(['tagsInput','fileLibrary','cascader'], function(e){
    ...
    c.vers_html = '{{# if(d.verify){ }}lay-verify="{{ d.verify }}" {{# } }}{{# if(d.vertype){ }}lay-verType="{{ d.vertype }}" {{# } }}';
    //表单元素
    c.text_html = '<input type="text" name="{{ d.name }}" value="{{ d.value }}" ' + c.vers_html + '{{# if(d.maxlength){ }}maxlength="{{ d.maxlength }}"{{# } }} placeholder="{{ d.placeholder || \'\' }}" ';
    c.number_html = '<input type="number" name="{{ d.name }}" value="{{ d.value }}" ' + c.vers_html + 'placeholder="{{ d.placeholder || \'\' }}" class="layui-input">';
    ...
    c.checkbox_html = '{{# layui.each(d.options, function(key, txt){ }}<input type="checkbox" name="{{ d.name }}[]" lay-filter="{{ d.name }}" lay-skin="{{ d.skin }}" value="{{ key }}" title="{{ txt }}" {{ (d.value).split(",").indexOf(key)>-1 ? "checked" : "" }}/>{{# }); }}';
    c.password_html = '<input type="password" name="{{ d.name }}" value="{{ d.value }}" ' + c.vers_html + 'autocomplete="off" placeholder="{{ d.placeholder || \'\' }}" ';
    c.textarea_html = '<textarea name="{{ d.name }}" class="layui-textarea" ';
    c.select_html = '<select name="{{ d.name }}" lay-filter="{{ d.name }}" ' + c.vers_html + '>{{# layui.each(d.options, function(key, txt){ }}<option value="{{ key }}" {{ d.value == key ? "selected" : "" }}>{{ txt }}</option>{{# }); }}</select>';
    //隐藏域
    c.hidden = '<input type="hidden" name="{{ d.name }}" value="{{ d.value || \'\' }}"/>';
    //静态代码
    c.html = c.item_html + c.block_html + '>' + '{{- d.html }}' + c.tips_html + '</div></div>';
    //单行文本
    c.text = c.item_html + c.block_html + '>' + c.text_html + '{{# if(d.tid){ }}id="{{ d.tid }}"{{# } }} class="layui-input"/>' + c.tips_html + '</div></div>';
    //多行文本、数组
    c.textarea = c.array = c.item_html + c.block_html + '>' + c.textarea_html + '{{# if(d.tid){ }}id="{{ d.tid }}" {{# } }}{{# if(d.style){ }}style="{{ d.style }}" {{# } }}' + c.vers_html + 'placeholder="{{ d.placeholder || \'\' }}">{{ d.value }}</textarea>' + c.tips_html + '</div></div>';
    //静态文本
    c.static = c.item_html + c.block_html + '>' + '<div class="layui-form-mid layui-word-aux">{{ d.value }}</div></div></div>';
    //密码
    c.password = c.item_html + c.block_html + '>' + c.password_html + '{{# if(d.tid){ }}id="{{ d.tid }}"{{# } }} class="layui-input"/>' + c.tips_html + '</div></div>';
    //复选
    ...
    //百度编辑器
    c.ueditor = c.item_html + c.block_html + '>' + c.textarea_html + ' id="ueditor-{{ d.tid ? d.tid : d.name }}" style="border:0;padding:0;">{{ d.value }}</textarea>' + c.tips_html + '</div></div>';
    //Md编辑器
    c.cherrymd = c.item_html + c.block_html + '><div id="cherrymd-{{ d.tid ? d.tid : d.name }}"></div><textarea id="temp-{{ d.tid ? d.tid : d.name }}" style="display:none;">{{ d.value }}</textarea>' + c.tips_html + '</div></div>';
    //Md编辑器
    c.editormd = c.item_html + c.block_html + '><style>.editormd-preview li{list-style:inherit!important}.editormd-code-toolbar>select{display:initial}</style><div id="editormd-{{ d.tid ? d.tid : d.name }}" style="z-index:1000;">' + c.textarea_html + ' style="display:none;">{{ d.value }}</textarea></div>' + c.tips_html + '</div></div>';
    //TinyMCE编辑器
    c.tinymce = c.item_html + c.block_html + '>' + c.textarea_html + ' id="tinymce-{{ d.tid ? d.tid : d.name }}">{{ d.value }}</textarea>' + c.tips_html + '</div></div>';
    //上传单图
    ...
    c.images = c.item_html + c.block_html + '><div id="image-show-{{ d.name }}" data-type="images" data-verify="{{ d.verify || \'\' }}">{{# var i = 0;layui.each(d.value, function(key, url){ i=1; }}'+
              ...
              '<div style="clear:both"></div>' + c.tips_html + '</div></div>';
    //上传文件
    c.upfile = c.item_html + c.block_html + '>' + c.inline_html + ' style="width:85px;float:right;"><button type="button" class="layui-btn" data-type="{{ d.filetype }}" id="{{ d.tid ? d.tid : \'upfile-btn-\' + d.name }}">上传文件</button></div>'+
    ...
    var b = {
        ...
        build: function(d){
            ...
            c.para = d.para || {}; //构建项json数据请求参数
            c.cpos = d.cpos || ''; //百度编辑器弹出层兼容全屏 fixed
            c.zdex = d.zdex || (d.cpos=='fixed' ? 19991000 : 999); //百度编辑器 zIndex
            if(c.data.length > 0 || c.data.constructor === Object){
                b.sett(c.data);
            }else{
                var load = layer.load(2);
                $.getJSON(c.url, c.para, function(res){
                    layer.close(load);
                    if(res.code === 1){
                        b.sett(res.data);
                    }else{
            ...
            $h.find("[id^='image-show-']").on('click','.imgs_item .item-delete', function(){
                ...
                layer.confirm('您确定要删除该' + (name ? name : '图片') + '吗？', {
                    ...
                    if(pparent.children().length==0){
                        ...
                        pparent.html('<input type="hidden" name="'+ name +'" lay-verify="'+ pparent.data('verify') +'"/>');
                    }
                    layer.close(index);
                });
            });
        ...
        uEditor: function(){
            ...
            if(ids.length > 0){
                b.getUE(function(){
                    for(var i in ids){
                        let eid = ids[i], heg = $("#"+eid).parent().parent().height() - 110; heg = heg>200 ? heg : 200;
                        UE.delEditor(eid);//先销毁，实现每次重新加载
                        UE.getEditor(eid,{
                            initialFrameHeight: heg,
                            toolbars: [[
        ...
        getTE: function(success){
            if(window.tinymce){
                typeof success === 'function' && (c.cpos ? setTimeout(function(){success()},500) : success());
            }else{
